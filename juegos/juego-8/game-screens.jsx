...
// R3 — Cazapuntuación (shooter): citas con narrativa larga y MUCHOS huecos.
//   Estructura: partes[] alterna entre texto fijo (t), slot (s) y separador (br = nueva línea).
//   La bandeja (globos) incluye distractores para que haya que pensar.
const CITAS_BANK = [
  {
    id: "profesor",
    partes: [
      { t: "Esa tarde de viernes, el profesor anunció con voz seria" }, { s: ":" }, { s: '"' },
      { t: "Mañana tendremos un examen muy importante" }, { s: '"' }, { s: "." },
      { br: true },
      { t: "Después, mirando a todos sus alumnos, agregó" }, { s: ":" }, { s: '"' },
      { t: "Por favor, estudien mucho esta noche" }, { s: '"' }, { s: "." },
    ],
    bandeja: [":", '"', ".", ",", "!", "?"],
  },
  {
    id: "abuelo",
    partes: [
      { t: "Durante el desayuno, el abuelo nos contó emocionado" }, { s: ":" }, { s: '"' },
      { t: "Mañana iremos juntos al río del pueblo" }, { s: '"' }, { s: "." },
      { br: true },
      { t: "Mi pequeña hermana saltó de alegría y exclamó" }, { s: ":" }, { s: '"' },
      { t: "¡Qué linda sorpresa nos dio el abuelo" }, { s: "!" }, { s: '"' },
    ],
    bandeja: [":", '"', "!", ".", ",", "?"],
  },
  {
    id: "novios",
    partes: [
      { t: "Cuando llegó el momento esperado, el novio respondió" }, { s: ":" }, { s: '"' },
      { t: "Sí, acepto con todo mi corazón" }, { s: '"' }, { s: "." },
      { br: true },
      { t: "La novia, sonriendo con ternura, le dijo" }, { s: ":" }, { s: '"' },
      { t: "Te amaré para toda la vida" }, { s: '"' }, { s: "." },
    ],
    bandeja: [":", '"', ".", ",", "!", "?"],
  },
  {
    id: "explorador",
    partes: [
      { t: "Al llegar a la cima de la montaña, el explorador gritó" }, { s: ":" }, { s: '"' },
      { t: "¡Por fin alcanzamos la cumbre nevada" }, { s: "!" }, { s: '"' },
      { br: true },
      { t: "Su compañero, agotado pero feliz, susurró" }, { s: ":" }, { s: '"' },
  ...
  },
  {
    id: "mama",
    partes: [
      { t: "Después de la cena del domingo, mamá nos preguntó" }, { s: ":" }, { s: '"' },
      { t: "¿Quién quiere ayudar a lavar los platos" }, { s: "?" }, { s: '"' },
      { br: true },
      { t: "Mi hermano levantó la mano y contestó" }, { s: ":" }, { s: '"' },
...
function EscritorGame({ app, setApp, go, onRestart }) {
  ...
  const zonaCentralTop =
    ...
    ronda === 1 ? 140 :  // R2 núcleo + chips flotantes + 2 carpetas ≈ 220px
  ...
  const Bucket = ({ tipo }) => {
    ...
    return (
      <div
        ...
        style={{
          minWidth: 200, padding: "10px 14px 12px 14px",
          background: cfg.bg,
...
function LabCitasCard({ pick, placed, picked, locked, onPickLabel, onPlaceIn, onSetPlaced }) {
  ...
  const renderCita = () => {
    ...
    return lineas.map((linea, lineaIdx) => (
      <div key={lineaIdx} style={{
        ...
        {linea.map((p) => {
          if (p.t) {
            return <span key={p._i} style={{ padding: "0 4px" }}>{p.t}</span>;
          }
  ...
  return (
    <div style={{ position: "relative", width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 36 }}>
      ...
      {/* Banco de GLOBOS — flotan con animación */}
      <div data-qa="bandeja" style={{
        ...
        display: "flex", flexWrap: "wrap", gap: 10, justifyContent: "center", alignItems: "flex-end",
        position: "relative",
      }}>
        <div style={{
...
function OrdenaCuentoCard({ cuento, order, selected, locked, onTap }) {
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 5, width: "fit-content", maxWidth: 430 }}>
        {order.map((escena, i) => {
