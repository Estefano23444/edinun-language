...
function ActionRail({ canVerify, onVerify, hideVerify, showErase, eraseDisabled, onErase, onRestart, onExit }) {
  return (
    <div data-qa="acciones" style={{
      ...
      {showErase && (
        <button className="ed-btn ed-btn-erase" onClick={onErase} disabled={eraseDisabled}
          style={{
            fontSize: 13, padding: "0 8px", height: 48, fontWeight: 800, letterSpacing: "0.04em",
            opacity: eraseDisabled ? 0.45 : 1, cursor: eraseDisabled ? "not-allowed" : "pointer",
          }}>
          BORRAR
        </button>
      )}
...
function FabulaGame({ app, setApp, go, onRestart }) {
  ...
  // Las opciones de moraleja se barajan por sesión para que la correcta NO quede
  // siempre en la misma posición (todas las del banco traen correcta:0). Se guarda
  // el TEXTO correcto antes de barajar y se recalcula su índice tras el shuffle, así
  // la calificación es por valor y no por una posición fija explotable.
  const [r2Opts] = useStateG(() => {
    const correctText = r2Pick.opciones[r2Pick.correcta];
    const opciones = shuffle(r2Pick.opciones);
    return { opciones, correcta: opciones.indexOf(correctText) };
  });
  ...
  function gradeR2(choice) {
    if (r2Locked) return;
    setR2Locked(true);
    const correct = choice === r2Opts.correcta;
    setTimeout(() => answer(correct, `Moraleja: ${r2Pick.titulo}`, "💡",
      r2Opts.opciones[choice], r2Opts.opciones[r2Opts.correcta]), 350);
  }
  ...
  function answer(isCorrect, reto, opIcon, userAnswer, correctAnswer) {
    ...
    const revealDelay = isCorrect ? 0 : 3000;   // tiempo para leer la respuesta correcta revelada al fallar (alineado con juego-7)
  ...
  const enunciado =
    ...
    "Adivina el personaje y forma su nombre.";
  const bocadillo =
  ...
  // Vista de R2 con las opciones ya barajadas (la rueda/cartel siguen usando los
  // campos originales vía spread: id para el sorteo, emoji/titulo/texto del cartel).
  const r2View = { ...r2Pick, opciones: r2Opts.opciones, correcta: r2Opts.correcta };
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      ...
      <div data-qa="zona-central" style={{
        ...
        {ronda === 1 && (
          <RuletaMoralejaCard pick={r2View} bank={MORALEJAS} spun={r2Spun} onLanded={() => setR2Spun(true)}
      ...
      <ActionRail canVerify={canVerify} onVerify={handleVerify} hideVerify={ronda === 1}
        showErase={showErase} eraseDisabled={ronda === 0 ? r1Locked : ronda === 2 ? r3Locked : false}
        onErase={handleErase}
              ...
              <div style={{ flex: 1, minWidth: 0, display: "flex", alignItems: "center" }}>
                {part ? (
                  <div
                    ...
                    {isWrong && (
                      <span style={{
                        flexShrink: 0, fontFamily: "var(--ed-font-display)", fontWeight: 800, fontSize: 10,
                        color: "#fff", background: "linear-gradient(180deg, #2ecc8f, #1f9c66)",
                        border: "1.5px solid #eafff4", borderRadius: 7,
                        padding: "2px 7px", letterSpacing: "0.03em", lineHeight: 1.15, whiteSpace: "nowrap",
                        boxShadow: "0 1px 5px rgba(0,0,0,0.3)",
                      }}>✓ Va en: {(PARTES_CFG.find((c) => c.id === part.tipo) || {}).label}</span>
                    )}
                  </div>
                ) : (
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12 }}>
      ...
      <div style={{
        ...
        padding: "10px 18px", boxShadow: "0 12px 26px rgba(0,0,0,0.5)", color: "#3a2608",
        display: "flex", flexDirection: "column", alignItems: "center", gap: 6,
      }}>
        <div style={{ fontSize: 50, lineHeight: 1 }}>{pick.emoji}</div>
      ...
      {/* Revelado de la palabra correcta al fallar (verde + ✓), igual que R2. */}
      {isWrong && (
        <div style={{
          fontFamily: "var(--ed-font-display)", fontWeight: 800, fontSize: 16,
          color: "#fff",
          background: "linear-gradient(180deg, #2ecc8f, #1f9c66)",
          border: "2px solid #eafff4", borderRadius: 999,
          padding: "5px 18px", letterSpacing: "0.02em",
          boxShadow: "0 4px 12px rgba(0,0,0,0.4), 0 0 0 3px rgba(46,204,143,0.3)",
        }}>✓ Era: {pick.word}</div>
      )}
