...
function pickFresh(bank, prefix) {
  ...
  // Bloquea la mitad más reciente del banco → deja la otra mitad como pool
  // fresco (convención estándar del repo: buena variedad sin agotar el pool).
...
const T2_OBRAS = [
  ...
  { id: "guitarra", parts: [
    { id: "gu1", n: 1, label: "Un músico llega a la plaza para dar una serenata al alcalde." },
    { id: "gu2", n: 2, label: "Se le rompen las cuerdas y canta con una olla y una cuchara." },
    { id: "gu3", n: 3, label: "El alcalde aplaude encantado y lo invita a la fiesta del pueblo." },
  ] },
];
...
function EntremesGame({ app, setApp, go, onRestart }) {
  ...
  function gradeR1(choice) {
    ...
    setTimeout(() => answer(correct, choice, r1Pick.answer, "🎭", "La ruleta de los personajes"), correct ? 450 : 1500);
  }
...
function CienciaFiccionGame({ app, setApp, go, onRestart }) {
  ...
  function gradeR2(choice) {
    ...
    setTimeout(() => answer(correct, choice, r2Pick.answer, "🔬", "El analista narrativo"), correct ? 450 : 1500);
  }

  // Califica la R3 con el veredicto tocado (misma rama que tenía handleVerify).
  function gradeR3(choice) {
    if (r3Locked) return;
    setR3Locked(true);
    const correct = (choice === "CONFIABLE") === r3Pick.confiable;
    setTimeout(() => answer(correct, choice, r3Pick.confiable ? "CONFIABLE" : "NO CONFIABLE", "🔍", "Detective de fuentes"), correct ? 450 : 1500);
  }
