...
const ZONES = {
  ...
  // Zona-central: en juego-5 ocupa más ancho (left:230 right:160 → ancho 510)
  // para dar más espacio a las mecánicas R0/R1 (emoji+palabra, cartas trivia).
  "zona-central":{ x: 224, y: 126, w: 520, h: 390 },
...
async function runForViewport(browser, vp, baseUrl, screenshotsDir) {
  ...
  try {
    ...
    if (await nameInput.count() > 0) {
      ...
      // juego-5 requiere seleccionar un tema (botón de nivel) antes de habilitar ENTRAR.
      // Tocamos el primer botón de tema disponible (Reglas C, S, Z por defecto).
      const levelBtn = page.locator('button:has-text("Reglas C, S, Z")').first();
