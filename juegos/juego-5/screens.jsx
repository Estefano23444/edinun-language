// screens.jsx — HomeScreen + CharacterScreen + CosmosBg para "Juego 5".
// Dos niveles: Reglas C, S, Z (7 años) · Lengua y pensamiento (13 años).
// Cada uno mapea a una categoría distinta que GameScreen lee para renderizar
// las 3 rondas con mecánicas diferentes.
...
function CosmosBg({ variant = "cosmic", glyphSize }) {
  // Tick reactivo: los sub-juegos cambian window.__currentChalkGlyphs y disparan
  // el evento `ed-chalk-glyphs-updated`. Sin este listener, el fondo se queda con
  // el set anterior (no se vuelve a renderizar al cambiar de nivel).
  const [, forceTick] = useState(0);
  useEffect(() => {
    function onUpd() { forceTick((n) => n + 1); }
    window.addEventListener("ed-chalk-glyphs-updated", onUpd);
    return () => window.removeEventListener("ed-chalk-glyphs-updated", onUpd);
  }, []);
  ...
  if (variant === "chalkboard") {
    const glyphs = (typeof window !== "undefined" && window.__currentChalkGlyphs) || [
      ...
      { c: "-ito", l: "10%", t: "78%", r: "12deg", s: "0.45em" },
      { c: "-ivo", l: "88%", t: "72%", r: "-10deg", s: "0.45em" },
      { c: "-azo", l: "3%",  t: "45%", r: "-4deg", s: "0.45em" },
      { c: "-oso", l: "92%", t: "45%", r: "8deg",  s: "0.45em" },
  ...
  // Cosmic (home / character / results) — glifos mixtos de los 2 niveles.
  return (
    <>
      <div className="ed-cosmos" />
      <div className="ed-glyphs" style={glyphsStyle}>
        <span style={{ left: "5%",  top: "10%", "--rot": "-8deg" }}>C</span>
        <span style={{ left: "84%", top: "6%",  "--rot": "6deg",  fontSize: "0.78em" }}>S</span>
        <span style={{ left: "92%", top: "72%", "--rot": "-12deg", fontSize: "0.74em" }}>Z</span>
        <span style={{ left: "3%",  top: "82%", "--rot": "12deg", fontSize: "0.85em" }}>💬</span>
        <span style={{ left: "46%", top: "4%",  "--rot": "-4deg", fontSize: "0.5em" }}>-azo</span>
        <span style={{ left: "7%",  top: "46%", "--rot": "-4deg", fontSize: "0.5em" }}>-ito</span>
        <span style={{ left: "88%", top: "40%", "--rot": "8deg",  fontSize: "0.65em" }}>🌍</span>
        <span style={{ left: "22%", top: "22%", "--rot": "10deg", fontSize: "0.6em" }}>✏</span>
        <span style={{ left: "70%", top: "24%", "--rot": "-6deg", fontSize: "0.5em" }}>-oso</span>
        ...
        <span style={{ left: "50%", top: "88%", "--rot": "4deg",  fontSize: "0.58em" }}>🤔</span>
...
// ─────────────────────────────────────────────────────────────
// Configuración de los 2 niveles. Cada nivel define su color,
...
const LEVELS_CFG = [
  ...
  {
    id: "lenguaPensamiento",
    label: "Lengua y pensamiento",
    grad: "linear-gradient(180deg, #7ab8ff, #2773d8)",
    ink: "#08264d",
    description: "Cosmovisión, globalización, prejuicios y estereotipos.",
    catLabel: "Lengua y pensamiento",
  },
];

// ─────────────────────────────────────────────────────────────
// 1. HomeScreen — logo + saludo + 2 botones de nivel + nombre + ENTRAR
// ─────────────────────────────────────────────────────────────
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      ...
      <div style={{
        ...
        {/* Formulario der: saludo + 2 botones de nivel + descripción + nombre */}
        <div style={{ display: "flex", flexDirection: "column", gap: 18, maxWidth: 520 }}>
          <div>
            <div className="ed-label" style={{ color: "#4fd8ff", marginBottom: 8 }}>
              De la letra al pensamiento
            </div>
          ...
          {/* 2 botones de tema — solo nombre del tema */}
          <div>
...
function CharacterScreen({ app, setApp, go }) {
  ...
  function choose() {
    const lvCfg = LEVELS_CFG.find((l) => l.id === (app.level || "csz")) || LEVELS_CFG[0];
    setApp((s) => ({
      ...s,
      character: sel,
