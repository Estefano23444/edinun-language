  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      ...
      <div data-qa="personaje" style={{
        ...
        <div data-qa="bocadillo" className="ed-float-soft" style={{
          ...
          <div style={{
            ...
            {ronda === 2 && <>No las toques todas:<br/>solo las que estén mal.</>}
            <div style={{
      ...
      <div data-qa="acciones" style={{
        ...
        {/* Las 3 rondas del nivel CSZ son AUTO-EVALUADAS (TOCA 1 OPCIÓN) →
            ninguna muestra VERIFICAR/BORRAR. */}
...
function CSZ_CompletaCard({ onAnswer, verifyRef, setVerifyReady, clearRef, setClearReady, setHintMsg }) {
  ...
  // Fase "reveal": al fallar, ANTES del overlay "¡UPS!" el hueco muestra la
  // letra correcta en verde (y la bandeja la marca con ✓). Bloquea entradas.
  const [reveal, setReveal] = useStateG(false);
  const REVEAL_MS = 2400;

  // AUTO-EVALUADA: al tocar una letra se califica sola (sin VERIFICAR/BORRAR),
  // igual que la R1. Si falla, revela la correcta en verde antes del "¡UPS!".
  ...
  // Esta ronda ya no usa VERIFICAR/BORRAR.
  ...
  function grade(letter) {
    ...
    if (letter === problem.correct) { onAnswer(letter, problem); return; }
    // Incorrecto: revelar la letra correcta y, tras REVEAL_MS, avisar al padre.
    setReveal(true);
    clearTimeout(autoRef.current);
    autoRef.current = setTimeout(() => onAnswer(letter, problem), REVEAL_MS);
  }
    ...
    <div style={{
        ...
        <span
          style={{
            ...
            background: reveal
              ...
              : picked ? "linear-gradient(180deg, #ffe97a, #d7b12a)" : "rgba(255,255,255,0.12)",
            border: `3px solid ${reveal ? "#2ecc8f" : "#f2c260"}`,
            color: reveal ? "#06381f" : picked ? "#3a2608" : "rgba(255,255,255,0.4)",
            fontSize: 38, fontWeight: 700,
            textAlign: "center",
            cursor: "default",
            transition: "all 0.15s ease",
            boxShadow: reveal
              ? "0 0 18px rgba(46,204,143,0.65), inset 0 1px 0 rgba(255,255,255,0.4)"
              : picked ? "0 0 0 4px rgba(252,233,168,0.4), inset 0 1px 0 rgba(255,255,255,0.5)" : "none",
          }}>{reveal ? problem.correct : (picked || "_")}</span>
      ...
      <div style={{ display: "flex", gap: 18, justifyContent: "center", marginTop: 14 }}>
        {["C", "S", "Z"].map((l) => {
          ...
          // Durante el revelado: la letra correcta se marca en verde con ✓,
          // la equivocada (la que tocó el niño) en rojo.
          const isRevealCorrect = reveal && l === problem.correct;
          const isRevealWrong = reveal && isPicked && l !== problem.correct;
          return (
            <button key={l} onClick={() => {
                if (reveal || lockedRef.current) return;
                setPicked(l);
                // Resalta y, si no cambia de idea en ~700 ms, califica sola.
                clearTimeout(autoRef.current);
                autoRef.current = setTimeout(() => grade(l), 700);
              }}
              style={{
                position: "relative",
                ...
                color: isRevealCorrect || isRevealWrong ? "#fff" : "#3a2608",
                fontFamily: "var(--ed-font-display)", fontWeight: 800, fontSize: 38,
                border: isRevealCorrect ? "3px solid #2ecc8f" : isPicked ? "3px solid #fce9a8" : "none",
                boxShadow: isRevealCorrect
                  ? "0 0 0 4px rgba(255,255,255,0.9), 0 0 22px rgba(46,204,143,0.8)"
                  : isPicked
                ...
                transform: (isPicked && !reveal) || isRevealCorrect ? "translateY(-3px) scale(1.05)" : "none",
                transition: "all 0.15s ease",
...
function CSZ_TriviaCard({ onAnswer, verifyRef, setVerifyReady, clearRef, setClearReady }) {
  ...
  // Si falla, revela primero la correcta (verde ✓) antes del "¡UPS!".
  function gradeR2(idx) {
    if (lockedRef.current) return;
    lockedRef.current = true;
    const isCorrect = idx === problem.correctIdx;
    setRevealed(true);
    clearTimeout(autoRef.current);
    autoRef.current = setTimeout(() => onAnswer(idx, problem), isCorrect ? 460 : REVEAL_MS);
  }
    ...
    <div style={{
      ...
      <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
        {problem.opciones.map((opt, i) => {
          ...
          const isCorrectOpt = i === problem.correctIdx;
          // Durante el revelado: la correcta en verde (✓), la mal elegida en rojo,
          // las demás atenuadas. Antes del revelado: estilo amarillo normal.
          let bg, border, boxShadow, color = "#3a2608", mark = "";
          if (revealed && isCorrectOpt) {
            bg = "linear-gradient(180deg, #9bf2c6, #2ecc8f)";
            border = "3px solid #d6fff0"; color = "#0b3a28"; mark = "✓ ";
            boxShadow = "0 0 0 4px rgba(255,255,255,0.9), 0 0 26px rgba(46,204,143,0.85), inset 0 1px 0 rgba(255,255,255,0.5)";
          } else if (revealed && isPicked) {
            bg = "linear-gradient(180deg, #ff9a9a, #ff6b6b)";
            border = "3px solid #ffd6d6"; color = "#5a0d0d";
            boxShadow = "0 0 0 4px rgba(255,255,255,0.7), 0 0 22px rgba(255,107,107,0.7), inset 0 1px 0 rgba(255,255,255,0.4)";
          } else if (revealed) {
            bg = "linear-gradient(180deg, #ffe97a, #d7b12a)"; border = "none";
            boxShadow = "inset 0 1px 0 rgba(255,255,255,0.4), inset 0 -3px 0 rgba(0,0,0,0.18), 0 6px 14px -4px rgba(0,0,0,0.45)";
          } else {
            bg = isPicked ? "linear-gradient(180deg, #ffd84b, #c0850c)" : "linear-gradient(180deg, #ffe97a, #d7b12a)";
          ...
          const lifted = (!revealed && isPicked) || (revealed && isCorrectOpt);
          const dimmed = revealed && !isCorrectOpt && !isPicked;
          return (
            <button key={i} onClick={() => {
              ...
              style={{
                ...
                transform: lifted ? "translateY(-4px) scale(1.04)" : "none",
                transition: "all 0.15s ease",
...
function CSZ_ShooterCard({ onFinish }) {
  ...
  useEffectG(() => {
    ...
    // Movimiento por requestAnimationFrame (~60 fps) con delta-time, en vez de
    // un setInterval a 40 ms (25 fps, se veía a saltos). La velocidad vy está
    // normalizada a px/seg para que sea igual de rápido independiente del fps.
    let rafId = 0;
    let lastTs = 0;
    const step = (ts) => {
      if (finishedRef.current) return;
      if (!lastTs) lastTs = ts;
      // dt en segundos, acotado para evitar saltos tras un frame perdido.
      ...
      setBubbles((bs) => {
        ...
        let perdidasDelta = 0;
        for (const b of bs) {
          // vy*60 px/seg (equivale al ritmo previo de vy*2.4 cada 40 ms).
          const newY = b.y + b.vy * 60 * dt;
          if (newY > 320) {
      ...
      {bubbles.map((b) => (
        <button key={b.id} onClick={() => handleTap(b)}
          style={{
            ...
            transform: `translateY(${b.y}px)`, willChange: "transform",
