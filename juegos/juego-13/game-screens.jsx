...
function makeProblemR2() {
  // 3 parejas frescas (FIFO por verbo) → 6 cartas (base + verbo), tablero 3×2.
  const recent = new Set(getRecent());
  let pool = R2_BANK.filter((p) => !recent.has("mem:" + p.verb));
...
function ReportajeGame({ app, setApp, go, onRestart }) {
  ...
  function handleVerify() {
    ...
    if (ronda === 0) {
      ...
      setTimeout(() => answer(correct, userText, correctText, "🫙", "prefijo a / en"), 380);
    }
  }
