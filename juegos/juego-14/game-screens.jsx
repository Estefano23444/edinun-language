...
function GameHUD({ elapsed, stars, attempted, solved, total = 3, roundResults = [], roundsTop = 58, app, setApp }) {
  ...
  return (
    <>
      ...
      <div style={{
        position: "absolute", top: roundsTop, left: "50%", transform: "translateX(-50%)",
...
// Ventana (ms) para cambiar la respuesta antes de que se auto-evalúe en las
// rondas de "1 toque = respuesta" sin botón VERIFICAR (N1 y N3-R1). Fuente única.
const RECONSIDER_MS = 1500;
...
function BibliotecaGame({ app, setApp, go, onRestart }) {
  ...
  // Auto-evaluación (sin VERIFICAR): al tocar una sílaba se califica tras
  // RECONSIDER_MS para poder cambiar de idea (tocar otra sílaba reinicia el
  // contador; tocar el hueco cancela). Mismo patrón que N2.
  ...
  // Califica la sílaba tocada (se llama desde el timer auto, no desde un botón).
  function gradeBib(syll) {
    if (feedback || reveal) return;
    if (syll !== problem.syll) {
      // Revela la correcta en verde ✓ (sin overlay) y luego muestra "¡UPS!".
      setReveal(true);
      setTimeout(() => {
        if (!aliveRef.current) return;
        setReveal(false);
        answer(false, syll, problem.syll, problem.emoji, problem.word);
      }, REVEAL_MS);
      return;
    }
    answer(true, syll, problem.syll, problem.emoji, problem.word);
  }
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      <GameHUD elapsed={elapsed} stars={stars} attempted={attempted} solved={solved} total={3} roundResults={log.map((e) => e.isCorrect)} app={app} setApp={setApp} />
      ...
      <div data-qa="zona-central" style={{
        ...
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: SLOT_GAP, padding: "4px 6px" }}>
          <button onClick={() => { if (picked && !feedback && !reveal) { clearTimeout(autoRef.current); setPicked(null); } }} style={{
          ...
          return (
            <button key={trayIdx} className="ed-numpad-key"
              onClick={() => {
                ...
                autoRef.current = setTimeout(() => gradeBib(syll), RECONSIDER_MS);
              }}
              style={{
      ...
      <ActionRail
        canVerify={false} hideVerify={true}
        showErase={false} onErase={undefined}
...
const R3_DET = [
  { id: "d1", lines: ["En el campo enamorado,", "el caballo va corriendo,", "canta el gallo colorado,", "y el río baja apurado."], intruder: 1 },
  { id: "d2", lines: ["Salió la luna brillante,", "el lucero va delante,", "la noche se puso oscura,", "y el sapo sale cantante."], intruder: 2 },
  { id: "d3", lines: ["Una niña en el sembrío", "recoge el maíz con brío,", "el agua corre en el río,", "cruzando todo el potrero."], intruder: 3 },
...
function PoemasGame({ app, setApp, go, onRestart }) {
  ...
  const spinTimerRef = useRefG(null);
  useEffectG(() => () => {
  ...
  function spinRuleta() {
    ...
    spinTimerRef.current = setTimeout(() => {
      if (!aliveRef.current) return;
      setR1Spinning(false); setR1Spun(true); exerciseStart.current = Date.now();
    }, 1750);
  }
  ...
  // No hay BORRAR en N2 (respuesta de 1 toque), así que tampoco hay handleErase.
...
const R1_FIG = [
  ...
  { id: "f11", verse: "Del río en la orilla descansaba el viajero.", figura: "hipérbaton" },
...
function VocesGame({ app, setApp, go, onRestart }) {
  ...
  // R1 (identificar la figura) es de "1 toque = respuesta": se auto-evalúa tras
  // RECONSIDER_MS sin botón VERIFICAR. R2/R3 (arrastrar varias fichas) SÍ llevan
  // VERIFICAR. Tocar otra figura reinicia el contador; tocar la misma cancela.
  const r1AutoRef = useRefG(null);
  useEffectG(() => () => clearTimeout(r1AutoRef.current), []);
  function gradeR1Auto(choice) {
    if (feedback || r1Locked) return;
    setR1Locked(true);
    const correct = choice === r1Pick.figura;
    setTimeout(() => answer(correct, choice, r1Pick.figura, "🔎", "Figura literaria"), correct ? 450 : 1500);
  }
  ...
  // BORRAR solo tiene sentido mientras la ronda activa no está bloqueada ni en
  // revelado; si no, sería un botón clicable que no responde durante el "¡UPS!".
  const roundLocked =
    (ronda === 0 && r1Locked) || (ronda === 1 && r2Locked) || (ronda === 2 && r3Locked);
  const canErase = !feedback && !roundLocked;
  ...
  function handleVerify() {
    ...
    if (ronda === 0) {
      ...
      // Reporte: mostrar el PAR completo (no solo la 1ª palabra) para que un
      // docente reconstruya el ejercicio; separador "; " para no chocar con el
      // " · " interno del par.
      const userText = r2Pick.fichas.map((f) => `${f.pair} → ${RIMA_LABEL[r2Placed[f.id]] || "?"}`).join("; ");
      const correctText = r2Pick.fichas.map((f) => `${f.pair} → ${RIMA_LABEL[f.tipo]}`).join("; ");
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "center" }}>
            {r1Opts.map((f, i) => (
              <OptionButton key={f} label={f} locked={r1Locked}
                ...
                onClick={() => {
                  if (feedback || r1Locked) return;
                  const next = r1Choice === f ? null : f;
                  setR1Choice(next);
                  clearTimeout(r1AutoRef.current);
                  if (next !== null) r1AutoRef.current = setTimeout(() => gradeR1Auto(next), RECONSIDER_MS);
                }}
                color={["#ef5a5a", "#4fa0ff", "#2ecc8f"][i % 3]} />
            ))}
          </div>
      ...
      <ActionRail
        ...
        showErase={ronda === 0 ? false : canErase} onErase={handleErase}
        onRestart={() => setConfirmingRestart(true)} onExit={() => setConfirmingExit(true)}
      />
      <FeedbackOverlay feedback={feedback} feedbackMsg={feedbackMsg} charName={char.name} />
      <ConfirmModal open={confirmingRestart} onClose={() => setConfirmingRestart(false)}
...
// Las dos frases nominales se distinguen por el TIPO de modificador del núcleo
// (adjetivo vs complemento preposicional), no por "ser o no frase nominal":
// ambas lo son. Por eso las etiquetas describen el modificador, no oponen
// "modificador" a "frase nominal" (oposición lingüísticamente falsa).
const ORA_LABEL = { modificador: "SUST. + ADJETIVO", interjeccion: "INTERJECCIÓN", nominal: "SUST. + COMPLEMENTO" };
...
function OpinionGame({ app, setApp, go, onRestart }) {
  ...
  // R1 (tipo de texto) y R2 (hecho/opinión) son de "1 toque = respuesta": se
  // auto-evalúan tras RECONSIDER_MS sin botón VERIFICAR. R3 (clasificar varias
  // oraciones) SÍ lleva VERIFICAR. Tocar otra opción reinicia el contador.
  const autoRef = useRefG(null);
  useEffectG(() => () => clearTimeout(autoRef.current), []);
  function gradeR1Auto(choice) {
    if (feedback || r1Locked) return;
    setR1Locked(true);
    const correct = choice === r1Pick.cat;
    setTimeout(() => answer(correct, CAT_OPT[choice], CAT_OPT[r1Pick.cat], "🕵️", "Tipo de texto"), correct ? 450 : 1500);
  }
  function gradeR2Auto(choice) {
    if (feedback || r2Locked) return;
    setR2Locked(true);
    const correct = (choice === "HECHO") === r2Pick.hecho;
    setTimeout(() => answer(correct, choice, r2Pick.hecho ? "HECHO" : "OPINIÓN", "⚖️", "Hecho u opinión"), correct ? 450 : 1500);
  }
  ...
  // BORRAR solo aplica mientras la ronda activa no está bloqueada ni en revelado.
  const roundLocked =
    (ronda === 0 && r1Locked) || (ronda === 1 && r2Locked) || (ronda === 2 && r3Locked);
  const canErase = !feedback && !roundLocked;
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      <GameHUD elapsed={elapsed} stars={stars} attempted={attempted} solved={solved} total={3} roundResults={log.map((e) => e.isCorrect)} roundsTop={96} app={app} setApp={setApp} />
      <CharacterCorner char={char} message={bocadillo} />
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "center" }}>
            {["informativo", "opinion", "mixto"].map((cat) => (
              <OptionButton key={cat} label={CAT_OPT[cat]} locked={r1Locked}
                ...
                onClick={() => {
                  if (feedback || r1Locked) return;
                  const next = r1Choice === cat ? null : cat;
                  setR1Choice(next);
                  clearTimeout(autoRef.current);
                  if (next !== null) autoRef.current = setTimeout(() => gradeR1Auto(next), RECONSIDER_MS);
                }}
                color={CAT_COLOR[cat]} />
            ))}
          </div>
        ...
        <div data-qa="zona-central" style={{
          ...
          <div style={{ display: "flex", gap: 16, justifyContent: "center" }}>
            <OptionButton label="HECHO" locked={r2Locked} isCorrect={r2Pick.hecho} isPicked={r2Choice === "HECHO"}
              onClick={() => {
                if (feedback || r2Locked) return;
                const next = r2Choice === "HECHO" ? null : "HECHO";
                setR2Choice(next);
                clearTimeout(autoRef.current);
                if (next !== null) autoRef.current = setTimeout(() => gradeR2Auto(next), RECONSIDER_MS);
              }} color="#4fa0ff" />
            <OptionButton label="OPINIÓN" locked={r2Locked} isCorrect={!r2Pick.hecho} isPicked={r2Choice === "OPINIÓN"}
              onClick={() => {
                if (feedback || r2Locked) return;
                const next = r2Choice === "OPINIÓN" ? null : "OPINIÓN";
                setR2Choice(next);
                clearTimeout(autoRef.current);
                if (next !== null) autoRef.current = setTimeout(() => gradeR2Auto(next), RECONSIDER_MS);
              }} color="#e0a23a" />
            ...
            {r3Pick.bins.map((bin) => {
              ...
              return (
                <div key={bin} data-dropzone={bin}
                  ...
                  {/* Al bloquear: si la ficha que pertenece a este cajón no está
                      aquí (el niño la puso en otro), se muestra su "fantasma" verde
                      con ✓ — así ve dónde la puso (rojo) Y dónde iba (verde). */}
                  {r3Locked && (() => {
                    const belongs = r3Pick.fichas.find((f) => f.tipo === bin);
                    if (!belongs || r3Placed[belongs.id] === bin) return null;
                    return (
                      <div style={{
                        fontFamily: "var(--ed-font-display)", fontWeight: 700, fontSize: 13,
                        padding: "6px 9px", borderRadius: 8, lineHeight: 1.1, textAlign: "center",
                        background: "rgba(46,204,143,0.16)", color: "#2ecc8f",
                        border: "2px dashed #2ecc8f", whiteSpace: "nowrap",
                      }} title="Esta era la que iba aquí">
                        ✓ {belongs.text}
                      </div>
                    );
                  })()}
      ...
      <ActionRail
        canVerify={canVerify} onVerify={handleVerify} hideVerify={ronda !== 2}
        showErase={ronda === 2 ? canErase : false} onErase={handleErase}
        onRestart={() => setConfirmingRestart(true)} onExit={() => setConfirmingExit(true)}
      />
      <FeedbackOverlay feedback={feedback} feedbackMsg={feedbackMsg} charName={char.name} />
      <ConfirmModal open={confirmingRestart} onClose={() => setConfirmingRestart(false)}
