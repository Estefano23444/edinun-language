...
const R2_BANK = [
  ...
  { id: "cuento", parts: [
    "Mi maestra leyó un cuento donde un león andaba de ", { options: ["caza", "casa"], answer: "caza" },
    " por el bosque. Llovía y el agua corría por un ", { options: ["tubo", "tuvo"], answer: "tubo" },
...
// Guardia de autoría: cada hueco debe tener answer ∈ options. Un typo (p. ej.
// answer:"caza" con options:["casa","caza"] mal escritas) rompería la
// calificación en silencio; esto lo avisa una vez al cargar, sin afectar el juego.
R2_BANK.forEach((row) => row.parts.forEach((p, i) => {
  if (p && typeof p === "object" && Array.isArray(p.options) && !p.options.includes(p.answer)) {
    console.error(`[juego-12] R2_BANK "${row.id}" hueco ${i}: answer "${p.answer}" no está en options ${JSON.stringify(p.options)}`);
  }
}));
    ...
    <>
      ...
      <div style={{
        ...
        {Array.from({ length: total }).map((_, i) => {
          // Colorea cada punto por el resultado REAL de SU ronda (no por conteo
          // acumulado): verde si esa ronda se acertó, rojo si se falló.
...
function ActionRail({ canVerify, onVerify, showVerify = true, hideVerify, showErase, onErase, onRestart, onExit }) {
  return (
    <div data-qa="acciones" style={{
      position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)",
      display: "flex", flexDirection: "column", gap: 10, width: 148,
    }}>
      {showVerify && !hideVerify && (
...
function FiestaGame({ app, setApp, go, onRestart }) {
  ...
  // R3 (clasificar formal/coloquial, 1 toque) se AUTO-EVALÚA: tras girar la
  // ruleta, al tocar "formal" o "coloquial" se resalta y, tras una breve
  // ventana para recapacitar (~700 ms), se califica sola (estándar de
  // mecánicas de 1 toque, igual que el shooter de R1). Por eso su VERIFICAR
  ...
  function gradeR3(choice) {
    if (r3Locked) return;
    setR3Locked(true);
    const correct = choice === r3Landed.type;
    const userText = TYPE_LABEL[choice];
    const correctText = `${TYPE_LABEL[r3Landed.type]} — "${r3Landed.text}"`;
    // Al fallar, deja ver el revelado (verde ✓ / rojo ✗) ~1.1 s antes del "¡UPS!".
    setTimeout(() => answer(correct, userText, correctText, "🎡"), correct ? 450 : 1100);
  }
  ...
  function handleShooterDone(caught, misses) {
    if (r1Locked) return;
    setR1Locked(true);
    // Puntaje = correctas atrapadas menos UNA por cada trampa tocada (penaliza
    // tocar las de "n"). Acierta solo si discriminó: 3+ correctas y margen 3+.
    const earned = Math.max(0, Math.min(10, caught - misses));
    const correct = caught >= 3 && (caught - misses) >= 3;
    const userText = `${caught} cazadas · ${misses} ${misses === 1 ? "fallo" : "fallos"}`;
    setTimeout(() => answer(correct, userText, "Cazar las de mp/mb y evitar las de n", "🎯", earned), 380);
  }
...
// las BIEN escritas (m antes de p/b) y deja caer las trampas con n. La ronda
// dura R1_TIME (20 s) y termina por temporizador (sin VERIFICAR); el puntaje
// son las correctas atrapadas menos una por cada trampa tocada.
// ─────────────────────────────────────────────────────────────
          ...
          return (
            <button key={w.id}
              ...
              style={{
                ...
                // Al bloquear (fin de ronda), congela el carril bajo el overlay.
