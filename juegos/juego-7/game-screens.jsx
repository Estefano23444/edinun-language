...
// R3 — Uso de la letra G: párrafo con 5 palabras incompletas; el niño elige G o J
// Reglas: -gencia/-gente/-ingente, -gia/-gio, -ger/-gir → G (siempre G, J es distractor)
const USO_G_BANK = [
  {
    id: "ente",
    titulo: "La gente y la inteligencia",
    regla: "-gente y -gencia se escriben con G",
    parrafo: [
      { t: "La " },
      { g: "ente",        antes: "" },   // → gente
      { t: " vio la inteli" },
      { g: "encia",       antes: "" },   // → inteligencia
      { t: " y la diri" },
      { g: "encia",       antes: "" },   // → dirigencia
      { t: " de la mujer. Por ne" },
      { g: "ligencia",    antes: "" },   // → negligencia
      { t: " perdió el contin" },
      { g: "ente",        antes: "" },   // → contingente
      { t: "." },
    ],
  },
  {
    id: "gia-gio",
    titulo: "La magia y el prestigio",
    regla: "-gia, -gio, -gioso se escriben con G",
    parrafo: [
      { t: "La ma" },
      { g: "ia" },           // → magia
      { t: " del presti" },
      { g: "io" },           // → prestigio
      { t: " vino de un acto prodi" },
      { g: "ioso" },         // → prodigioso
      { t: ". Encontraron un refu" },
      { g: "io" },           // → refugio
      { t: " y todos hicieron un elo" },
      { g: "io" },           // → elogio
      { t: "." },
    ],
  },
  {
    id: "ger-gir",
    titulo: "Recoger y dirigir",
    regla: "Verbos en -ger y -gir se escriben con G",
    parrafo: [
      { t: "Hay que reco" },
      { g: "er" },           // → recoger
      { t: " la tarea, exi" },
  ...
  },
  {
    ...
    regla: "Las palabras en -gencia y -gente se escriben con G",
    parrafo: [
      { t: "Se comunica a toda la " },
      { g: "ente" },           // gente
      { t: " que por la ne" },
      { g: "ligencia" },       // negligencia
      { t: " de algunos, no habrá indul" },
      { g: "encia" },          // indulgencia
      { t: ". Vamos a corre" },
      { g: "ir" },             // corregir
      { t: " esto. Los diri" },
  ...
  },
  {
    id: "estudios",
    titulo: "Mis estudios",
    regla: "Las palabras en -logía y -gioso se escriben con G",
    parrafo: [
      { t: "Estudio bio" },
      { g: "ía" },             // biología
      { t: " y geo" },
      { g: "ía" },             // geografía → geo + gía
      { t: ". Mi profesor es pro" },
      { g: "ioso" },           // prodigioso
      { t: " explicando. La re" },
      { g: "ión" },            // región
      { t: " que estudiamos tiene mucha eco" },
      { g: "ía" },             // ecología
      { t: "." },
    ],
  },
...
function GameEnunciado({ text }) {
  ...
  return (
    <div style={{
      position: "absolute", top: 84, left: 244, width: 488,
      textAlign: "center",
...
function ResenaGame({ app, setApp, go, onRestart }) {
  ...
  const bocadillo =
    ronda === 0 ? "Arrastra cada frase al cajón." :
    ronda === 1 ? "1 tap = cardinal, 2 = ordinal, 3 = indefinido." :
...
// ─────────────────────────────────────────────────────────────
function PintorDeterminantesCard({ pick, colors, selectedTipo, locked, onSelectTipo, onApply }) {
  const TIPOS = [
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10 }}>
      {/* 3 botones de TIPO — seleccionables */}
      <div style={{ display: "flex", gap: 8 }}>
        {TIPOS.map((t) => {
          const isSel = selectedTipo === t.id;
          return (
            <button key={t.id}
              onClick={() => !locked && onSelectTipo(t.id)}
              disabled={locked}
              style={{
                display: "flex", flexDirection: "column", alignItems: "center", gap: 2,
                background: isSel ? "linear-gradient(180deg,#fce9a8,#d9a441)" : t.color,
                color: isSel ? "#3a2608" : "#fff",
                padding: "5px 16px", borderRadius: 12,
                fontFamily: "var(--ed-font-display)", fontWeight: 800, fontSize: 11.5,
                border: `2px solid ${isSel ? "#4fd8ff" : "rgba(255,255,255,0.7)"}`,
                boxShadow: isSel ? "0 0 16px rgba(79,216,255,0.7)" : "0 3px 7px rgba(0,0,0,0.3)",
              ...
              <span>{t.label}</span>
              <span style={{ fontSize: 8.5, opacity: 0.9, fontStyle: "italic", letterSpacing: 0 }}>
                {t.hint}
              </span>
            </button>
          );
      ...
      {/* Cartel del texto con determinantes */}
      <div style={{
              ...
              <span key={i}
                ...
                {(isWrong || isEmpty) && correctTipo && (
                  <sup style={{ fontSize: 9, color: "#c33b3b", marginLeft: 2, fontWeight: 800 }}>
                    ✗ Era {correctTipo.label.charAt(0)}
                  </sup>
                )}
              </span>
            );
...
function PartesResenaCard({ pick, parrafos, assigned, selectedBin, locked, onSelectBin, onAssignParrafo }) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", gap: 6, paddingTop: 0 }}>
      {/* Título del libro */}
      <div style={{
        display: "flex", alignItems: "center", gap: 8,
        background: "rgba(10,6,35,0.55)",
        ...
        borderRadius: 999,
        padding: "3px 14px",
        fontFamily: "var(--ed-font-display)", fontWeight: 700, fontSize: 11.5, color: "#fce9a8",
      }}>
        {pick.portada} {pick.titulo}
      </div>
      ...
      <div data-qa="bandeja" style={{
        ...
        {pendientes.length === 0 ? (
          <div style={{
            textAlign: "center", color: "rgba(252,233,168,0.6)",
            fontFamily: "var(--ed-font-display)", fontStyle: "italic", fontSize: 11,
            padding: "8px 4px",
          }}>
            ¡Bien! Todas las frases ya están clasificadas.
          </div>
        ) : pendientes.map((p) => (
          <div key={p.id}
            className="ed-draggable"
...
function EscritorGame({ app, setApp, go, onRestart }) {
  ...
  // R3 state: { verboIndex: color "azul" | "rojo" | null }
  ...
  function handleVerify() {
    ...
    if (ronda === 0) {
      ...
      for (const h of r3Huecos) {
        // Todas las palabras del banco USO_G_BANK son con G (las reglas del libro)
        if (r3Colors[h.idx] !== "g") { correct = false; }
      }
      const userText = r3Huecos.map((h) => `_${h.g}=${r3Colors[h.idx] || "?"}`).join(", ");
      const correctText = r3Huecos.map((h) => `g${h.g}`).join(", ");
      setTimeout(() => answer(correct, userText, correctText, "🔠"), 350);
    }
  }
  ...
  const bocadillo =
    ...
    ronda === 1 ? "Toca una terminación y luego un cajón." :
    "1 tap = G, 2 taps = J. Llena cada hueco.";
        ...
        {ronda === 2 && (
          <DetectiveGCard pick={r3Pick}
            ...
            onCycle={(idx) => {
              ...
              const next = cur === undefined ? "g" : cur === "g" ? "j" : undefined;
              const newColors = { ...r3Colors };
              if (next === undefined) delete newColors[idx]; else newColors[idx] = next;
              setR3Colors(newColors);
            }}
          />
        )}
      </div>
                    ...
                    return (
                      <button key={op}
                        ...
                        style={{
                          ...
                          background: isPicked ? "linear-gradient(180deg,#fce9a8,#d9a441)" : "rgba(255,255,255,0.95)",
                          color: "#3a2608",
                          border: `1.5px solid ${isPicked ? "#4fd8ff" : "rgba(242,194,96,0.55)"}`,
                          fontFamily: "var(--ed-font-display)", fontWeight: 700, fontSize: 12,
                          cursor: locked ? "default" : "pointer",
                          boxShadow: isPicked ? "0 0 12px rgba(79,216,255,0.55)" : "0 2px 5px rgba(0,0,0,0.25)",
                          transform: isPicked ? "translateY(-1.5px)" : "none",
                          transition: "all 0.15s",
                          minWidth: 76,
                        }}>
                        {op}
...
// ─────────────────────────────────────────────────────────────
function DetectiveGCard({ pick, choices, locked, onCycle }) {
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 9 }}>
      ...
      <div style={{
        ...
        <div style={{ fontFamily: "var(--ed-font-display)", fontWeight: 600, fontSize: 16, lineHeight: 2, textAlign: "center" }}>
          {pick.parrafo.map((tk, i) => {
            if (tk.t) return <span key={i}>{tk.t}</span>;
            const choice = choices[i]; // "g" | "j" | undefined
            const isCorrect = locked && choice === "g";
            const isWrong = locked && choice === "j";
            const isEmpty = locked && !choice;
            let bg = "rgba(252,233,168,0.85)";
            let border = "2px dashed #d9a441";
            let color = "#3a2608";
            if (choice === "g") { bg = "rgba(46,204,143,0.35)"; border = "2px solid #2ecc8f"; }
            if (choice === "j") { bg = "rgba(255,107,107,0.35)"; border = "2px solid #ff6b6b"; }
            ...
            return (
              <span key={i}
                ...
                <span style={{
                  ...
                  {choice ? choice.toUpperCase() : "_"}
                </span>
                <span style={{ fontWeight: 700 }}>{tk.g}</span>
                {(isWrong || isEmpty) && (
                  <sup style={{ fontSize: 10, color: "#c33b3b", marginLeft: 2, fontWeight: 800 }}>✗ Era G</sup>
                )}
              </span>
            );
          })}
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <div style={{
          display: "flex", alignItems: "center", gap: 6,
          background: "rgba(46,204,143,0.85)",
          color: "#fff",
