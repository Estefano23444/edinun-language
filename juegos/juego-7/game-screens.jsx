...
//   1. La reseña             (8 años)  — partes de la reseña / pintor de determinantes / tabla del verbo
//   2. Aprendiendo a escribir (10 años) — pronombre detective / laboratorio verboides / detective G-J
//
...
// FIFO por categoría. Cada clave la comparten varios bancos (una entrada por
// ronda y partida): "resena" la usan 3 rondas (3 bancos → 3 ids/partida) y
// "escritor" 2 rondas (2 ids/partida). Con un tope bajo la ventana efectiva por
// banco caía a ~1; subimos los topes para conservar ~3 recientes por banco
// (resena: 3 bancos × ~3, escritor: 2 bancos × ~3). usog es de un solo banco de
// 16 escenas, 8 recientes dejan 8 frescas.
const RECENT_LIMITS = { resena: 12, escritor: 9, usog: 8 };
...
const PRONOMBRES_BANK = [
  ...
  },
  {
    ...
    huecos: [
      ...
      { id: "h3", sustantivo: "La bicicleta de mi prima", pronombre: "Suya",   opciones: ["Suya", "Tuya", "Mía"] },
    ],
  },
  {
    id: "clase",
...
const USO_G_BANK = [
  ...
  },
  {
    ...
    parrafo: [
      { t: "Vimos mucha ve" },
      { c: "g", g: "etación" },                 // vegetación (ve + g + etación, g suave ante e)
      { t: " verde. Recogimos ho" },
      { c: "j", g: "as" },                      // hojas (ho + j + as)
...
//   R1: Partes de la reseña (clasificar párrafos en 3 cajones)
//   R2: Pintor de determinantes (tap cicla cardinal/ordinal/indefinido)
//   R3: Tabla del verbo (arrastrar desinencias por persona)
// ─────────────────────────────────────────────────────────────
function ResenaGame({ app, setApp, go, onRestart }) {
  ...
  const r3AllPlaced = Object.keys(r3Placed).length === r3Pick.filas.length;
        ...
        {ronda === 0 && (
          <PartesResenaCard pick={r1Pick} parrafos={r1Parrafos}
            ...
            onAssignParrafo={(pid, zoneId) => {
              if (r1Locked) return;
              if (zoneId === "__bandeja__") {
                // Devolver la frase a la bandeja arrastrándola allí.
                const next = { ...r1Assigned }; delete next[pid];
                setR1Assigned(next); setR1SelectedBin(null);
                return;
              }
              if (zoneId) {
          ...
          {pick.parrafo.map((tk, i) => {
            ...
            return (
              <span key={i}
                ...
                {tk.d}
                {/* Al fallar, muestra lo que eligió el niño (✗ rojo) Y lo correcto (✓ verde). */}
                {isWrong && (
                  <sup style={{ fontSize: 9.5, color: "#c0392b", marginLeft: 3, fontWeight: 800, whiteSpace: "nowrap" }}>
                    ✗ {(TIPOS.find((t) => t.id === color) || {}).label}
                  </sup>
                )}
                {(isWrong || isEmpty) && correctTipo && (
                  <sup style={{ fontSize: 9.5, color: "#1e8a5d", marginLeft: 3, fontWeight: 800, whiteSpace: "nowrap" }}>
                    ✓ {correctTipo.label}
                  </sup>
                )}
              </span>
            ...
            <div key={f.id}
              ...
              <div
                ...
                style={{
                  ...
                  background: des
                    ? (isWrong ? "linear-gradient(180deg,#ff8888,#e04545)" : "linear-gradient(180deg, #4666c8, #243f96)")
                    : "rgba(10,6,35,0.1)",
                  border: `2px ${des ? "solid" : "dashed"} ${des ? (isWrong ? "#ff6b6b" : "#6f8ee8") : "rgba(116,72,32,0.5)"}`,
                  borderRadius: 8,
                  display: "flex", alignItems: "center", justifyContent: "center",
...
function PartesResenaCard({ pick, parrafos, assigned, selectedBin, locked, onSelectBin, onAssignParrafo }) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-around", gap: 6, paddingTop: 4, paddingBottom: 4 }}>
            ...
            <div key={b.id}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 3, flex: 1 }}>
                {parrafosEnCajon.length === 0 ? (
                  // En locked NO mostramos el placeholder de acción (se veía como
                  // si el cajón siguiera interactuable/vacío durante el revelado).
                  locked ? null : (
                  <div style={{
      ...
      {/* Bandeja de párrafos pendientes — arrastrables. También es dropzone:
          soltar aquí una frase ya clasificada la devuelve a pendientes. */}
      <div data-qa="bandeja" data-dropzone="__bandeja__" style={{
...
function EscritorGame({ app, setApp, go, onRestart }) {
  ...
  // R1 (pronombre, toca-opción) se AUTO-EVALÚA: cuando los 3 huecos quedan
  // elegidos, una breve ventana (~700 ms) permite recapacitar y luego se
  // califica sola. Por eso su VERIFICAR se oculta y su canVerify queda en false.
  const canVerify =
    ronda === 0 ? false :
    ronda === 1 ? (r2AllPlaced && !r2Locked) :
    ronda === 2 ? (r3AllPainted && !r3Locked) :
    false;
  // En R1 ya no hay BORRAR (no hace falta sin VERIFICAR); resto de rondas sí.
  const showErase = ronda !== 0;

  const r1AutoRef = useRefG(null);
  useEffectG(() => () => clearTimeout(r1AutoRef.current), []);

  // Califica la R1 con las selecciones actuales (mismo flujo que tenía VERIFICAR).
  function gradeR1(sels) {
    if (r1Locked) return;
    setR1Locked(true);
    let correct = true;
    for (const h of r1Pick.huecos) {
      if (sels[h.id] !== h.pronombre) { correct = false; }
    }
    const userText = r1Pick.huecos.map((h) => `${h.sustantivo}=${sels[h.id] || "?"}`).join(", ");
  ...
  function handleErase() {
    // R1 (pronombre) se auto-evalúa y no muestra BORRAR.
    if (ronda === 1) {
      if (r2Locked) return;
      setR2Placed({});
      setR2Picked(null);
  ...
  const enunciado =
    ronda === 0 ? `Elige el pronombre correcto para cada caso.` :
        ...
        {ronda === 0 && (
          <PronombreCard pick={r1Pick}
            ...
            onChoose={(huecoId, pronombre) => {
              if (r1Locked) return;
              const next = { ...r1Selections, [huecoId]: pronombre };
              setR1Selections(next);
              // Cuando los 3 huecos están elegidos, califica sola tras una breve
              // ventana para recapacitar. Cualquier nuevo toque reprograma el
              // temporizador, así el niño puede cambiar de idea (sin toggle).
              clearTimeout(r1AutoRef.current);
              if (r1Pick.huecos.every((h) => next[h.id])) {
                r1AutoRef.current = setTimeout(() => gradeR1(next), 700);
              }
            }}
          />
        )}
          ...
          return (
            <div key={b.id}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, flex: 1, justifyContent: "center" }}>
                ...
                {(isWrong || isEmpty) && (
                  // Revelado de la desinencia CORRECTA en VERDE con ✓ (lo que el
                  // niño puso ya se ve en rojo/tachado en el slot de la izquierda).
                  <div style={{
                    fontFamily: "var(--ed-font-display)", fontWeight: 800, fontSize: 12,
                    color: "#fff",
                    background: "linear-gradient(180deg, #6fe0b0, #2ecc8f)",
                    padding: "4px 10px", borderRadius: 999,
                    border: "1.5px solid #d6fff0",
                    boxShadow: "0 3px 7px rgba(0,0,0,0.3)",
                    letterSpacing: "0.04em",
                  }}>
                    ✓ {pick.correctas[b.id]}
        ...
        <div style={{ fontFamily: "var(--ed-font-ui)", fontWeight: 400, fontSize: 17, lineHeight: 2.5, textAlign: "center" }}>
          {groups.map((g, j) => {
            ...
            const renderHalf = (letter, fillColor) => {
              ...
              // Al bloquear, el color va por CORRECCIÓN (no por letra): la letra
              // correcta SIEMPRE verde, la que el niño eligió mal en rojo.
              let bg = "transparent", col = "rgba(116,72,32,0.55)";
              if (locked) {
                if (letter === correct) { bg = "#2ecc8f"; col = "#fff"; }
                else if (isSel) { bg = "#ff6b6b"; col = "#fff"; }
              } else if (isSel) {
                bg = fillColor; col = "#fff";
              }
              return (
                <span
            ...
            return (
              <span key={`w-${j}`} style={{ display: "inline-block", margin: 0, userSelect: "none", whiteSpace: "nowrap" }}>
                ...
                {(isWrong || isEmpty) && (
                  <sup style={{ fontSize: 10, color: "#1e8a5d", marginLeft: 2, fontWeight: 900 }}>✓{correct}</sup>
                )}
              </span>
            );
          })}
